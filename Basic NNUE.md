...
## Quantization
We could use floats for calculating the evaluation, but it has two major problems: when you do a lot of updates, the error from floats accumulates and can get quite big. The second problem is that `float` is usually 32 bits wide, and because you will want to use SIMD for the net, and narrowing the numbers to 16 or 8 bit will result in much faster code. Because of these problems, we do an important trick: **quantization**.
...
$a1 = a \cdot Q$
$c1 = \frac{a1 \cdot b}{Q}$
which is almost the same as $c$, and there are no floats anymore.
...
Hidden layer neuron value is $x1 = v \cdot w0 + b0$, and activation is $a1 = \text{clamp}(x1, 0, 1)$. Output neuron value is $o = a1 \cdot w1 + b1 = \text{clamp}(v \cdot w0 + b0, 0, 1)^2 \cdot w1 + b1$
