...
```
Data should be distributed approximately equally between buckets. However, there's much less data when the king is on some squares. That's why there are a lot of squares that belong to bucket $3$.
The implementation is very similar to Mirroring: every time the king changes its bucket, you recalculate the accumulators from scratch. You also calculate the accumulator for a color based on that color's king position. Because you need a separate $w0$ matrix for every bucket, the number of weights increases almost proportionally with the number of buckets, so you need much more data for them to work. Generally, you start with 4 buckets and gradually increase their number, testing if it gains every time.