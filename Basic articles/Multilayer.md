...
However, there is a clever algorithm (mentioned below) which allows us to evaluate the net without a considerable performance hit.
...
Just like in output buckets, the number of weights doesn't increase a lot, so we don't need more data to train a multilayer net.
