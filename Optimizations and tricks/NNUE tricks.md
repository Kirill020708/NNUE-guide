...
The size of the first hidden layer becomes $N$ instead of $2N$, so we make the inference approximately 2 times faster. We lose some accuracy, but we gain too much speed to care. This technique is usually used with multilayer networks, but you can try it with a single-layer net.
